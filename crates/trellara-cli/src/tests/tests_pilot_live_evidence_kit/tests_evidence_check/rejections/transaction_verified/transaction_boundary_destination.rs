...
#[test]
fn pilot_evidence_check_rejects_transaction_boundary_with_publish_destination_mismatch() {
    let root = temp_root("pilot-evidence-check-source-ack-destination-mismatch");
    ...
    .expect("write transaction boundary with destination mismatch");
    ...
    assert_eq!(
        ...
        vec!["source ack publish destinations".to_string()]
    );

    fs::remove_dir_all(root).expect("remove evidence check destination mismatch temp dir");
}
